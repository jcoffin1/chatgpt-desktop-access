...
def audit(projectRoot, packagePath, version):
	...
	assert 'self._queueConversationModeProbe("requested mode switch activation", activate=True)' in plugin, "mode switch does not use the UI Automation worker"
	...
	assert "pattern.QueryInterface(UIAHandler.IUIAutomationInvokePattern)" in plugin, "native mode-menu-item Invoke pattern missing"
	assert "UIAHandler.UIA_MenuItemControlTypeId" in plugin, "native mode menu items are not queried"
	assert 'self._pendingConversationModeSwitch = "detecting"' in plugin, "rapid mode-switch gestures can invoke the control twice"
	assert "pendingProbe != activationProbe" in plugin, "native mode events can invalidate a requested switch before confirmation"
